...
Ejemplo
  Hora en Costa Rica: 2025-07-27 13:00:00
  Hora en España: 2025-07-27 21:00:00
  Días restantes para "31/12/2025": 157 días
...
2. Despliega el template de s3-bucket.yaml
Este es el primer stack que se debe crear. Una vez creado se deben copiar todos los archivos de la carpeta *application* al root del s3 bucket. El bucket generado tiene el nombre time-app-backend-bucket, si se desea cambiar el nombre se deben modificar los archivos de core-infra.yaml y boostrap.sh por lo que no se recomienda modificarlo si no se tiene experiencia con archivos yaml y scripts de shell.

3. Despliega el template core-infra.yaml
...
Descripción: ID de la VPC donde se van a crear los recursos. Tiene que ser un valor con la forma vpc-xxxxxxxxxxxxxxxxx. Para efectos del proyecto se van a usar las subredes y la vpc por default presentes en el learner lab ya que cumplen con lo que necesitamos para el proyecto. Si el usuario no tiene una VPC y las dos subredes solicitadas debe crearlas por aparte ya que no se cubre en las plantillas facilitadas.  Nota: Cuando un parámetro indique "-xxxxxxxxxxxxxxxxx", en la realidad eso va a ser una combinación de letras y números exclusivo de cada cuenta de AWS.
...
4. Despliega el template de sns-and-alerting.yaml
...
### EC2 (Instances Web en Auto Scaling Group)

Tipo: t2.micro (dentro del free tier) o t3.micro (si el free tier ya no aplica).

Precio promedio:

t2.micro: $0.0116/hora ≈ $8.50/mes por instancia

Escenario con 2 instancias activas = $17/mes

Nota: Si se usan Spot Instances, el precio puede bajar hasta 70%.

### Elastic Load Balancer (ALB)

Costo base: $0.0225/hora ≈ $16.50/mes

Costo adicional por tráfico: $0.008 per LCU/hour y por GB procesado (~$0.008/GB).

Estimación total (uso moderado): $20–25/mes

### Amazon S3 (para almacenamiento de archivos y código)

Bucket: backend-s3-bucket

Uso bajo (código y archivos bootstrap):

  < 1 GB almacenado + < 1,000 solicitudes PUT/GET ≈ $0.10–0.20/mes

### CloudFormation

Gratis. Solo se cobra por los recursos que crea.

### WAF (Web Application Firewall)

Precio base: $5.00/mes por ACL web

Precio adicional: $1.00 por regla activa (en este caso: regla bloqueadora predeterminada)

Estimación total: $6.00/mes

### EBS (volúmenes asociados a EC2)

Volumen de 8 GB (por defecto en AMI Amazon Linux 2)

Precio: $0.10/GB/mes ⇒ 8 GB ≈ $0.80/instancia/mes

Para 2 instancias: $1.60/mes

Total Aproximado	$45 – $50 USD/mes

### Reducción de costos posibles

1. Usar instancias Spot para reducir el costo EC2.

2. Aplicar Auto Scaling con mínimo = 1 y máximo = 2.
